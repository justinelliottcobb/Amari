use amari_tropical::{TropicalNumber, TropicalMatrix, TropicalMultivector};
...
mod tropical_tests {
    ...
    fn test_tropical_softmax_becomes_max() {
        ...
        let traditional_argmax = logits.iter()
            .enumerate()
            .max_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap())
            .unwrap().0;
        
        // Tropical max (just find max)
        let tropical_max = tropical_logits.iter()
            .enumerate()
    ...
    fn test_tropical_viterbi_path() {
        ...
        let transitions = TropicalMatrix::from_log_probs(&[
            vec![-1.0, -2.0],
            vec![-1.5, -0.5],
        ]);
        
        // Emission matrix
        let emissions = TropicalMatrix::from_log_probs(&[
            vec![-0.5, -2.0, -1.0],
            vec![-2.0, -0.5, -1.5],
        ]);
