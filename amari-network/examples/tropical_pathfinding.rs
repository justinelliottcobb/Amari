...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (start, end, start_name, end_name) in test_routes {
        ...
        match tropical_network.shortest_path_tropical(start, end)? {
            Some((path, distance)) => {
                print!("    Path: ");
        ...
        match network.shortest_path(start, end)? {
            Some((std_path, std_distance)) => {
                if std_path != path.unwrap_or_default() {
                    println!("    🔄 Standard algorithm found different path: {:?} ({:.1}h)",
                             std_path.iter().map(|&i| city_indices[i].1).collect::<Vec<_>>(),
                             std_distance);
                }
            }
            None => {
    ...
    println!("✅ Computed shortest paths between all {} city pairs", cities.len());
    ...
    println!("  • Network connectivity: {:.1}% ({}/{} pairs reachable)",
             connectivity, reachable_pairs, total_pairs);
    println!("  • Average travel time: {:.1} hours", average_distance);
    println!("  • Longest route: {} → {} ({:.1} hours)",
             city_indices[max_distance_pair.0].1,
             city_indices[max_distance_pair.1].1,
             max_distance);
