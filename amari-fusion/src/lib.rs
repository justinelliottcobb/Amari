...
impl<T: Float, const DIM: usize> TropicalDualClifford<T, DIM> {
    ...
    pub fn random() -> Self {
        let logits: Vec<T> = (0..DIM)
            .map(|_| T::from(fastrand::f64() * 2.0 - 1.0).unwrap())
            .collect();
    ...
    pub fn random_with_scale(scale: T) -> Self {
        let logits: Vec<T> = (0..DIM)
            .map(|_| scale * T::from(fastrand::f64() * 2.0 - 1.0).unwrap())
            .collect();
        Self::from_logits(&logits)
    }
    ...
    pub fn from_logits(logits: &[T]) -> Self {
        ...
        let dual_coeffs_t: Vec<T> = logits
            .iter()
        ...
        let dual_coeffs_f64: Vec<f64> = dual_coeffs_t
            .iter()
    ...
    fn compute_combined_score(&self, other: &Self) -> T {
        ...
        let tropical_contrib = self
            .tropical
            .max_element()
            .tropical_add(other.tropical.max_element());
        ...
        let clifford_contrib =
            T::from(self.clifford.scalar_product(&other.clifford)).unwrap_or(T::zero());
    ...
    pub fn distance(&self, other: &Self) -> T {
        ...
        let tropical_dist =
            (self.tropical.max_element().value() - other.tropical.max_element().value()).abs();

        // Dual distance (with derivative information)
        let dual_dist =
            T::from((self.dual.clone() - other.dual.clone()).norm().real).unwrap_or(T::zero());

        // Clifford distance (geometric norm)
        let clifford_dist =
            T::from((self.clifford.clone() - other.clifford.clone()).norm()).unwrap_or(T::zero());

        // Combined distance metric
        (tropical_dist * tropical_dist + dual_dist * dual_dist + clifford_dist * clifford_dist)
            .sqrt()
    }
...
impl<T: Float> SensitivityMap<T> {
    ...
    pub fn most_sensitive(&self, n: usize) -> Vec<usize> {
        let mut indexed: Vec<(usize, T)> = self
            .sensitivities
            .iter()
...
impl<T: Float> TropicalDualDistribution<T> {
    ...
    pub fn geometric_alignment(&self, reference: &Self) -> f64 {
        self.logits
            .clifford
            .scalar_product(&reference.logits.clifford)
    }
...
pub mod conversion {
    ...
    pub fn softmax_to_tropical<T: Float>(probs: &[T]) -> Vec<TropicalNumber<T>> {
        probs
            .iter()
            .map(|&p| TropicalNumber::from_log_prob(p.ln()))
            .collect()
    }
    ...
    pub fn clifford_to_dual<T: Float>(mv: &Multivector<3, 0, 0>) -> Vec<DualNumber<T>> {
        (0..8)
            .map(|i| DualNumber::variable(T::from(mv.get(i)).unwrap_or(T::zero())))
            .collect()
    }
}
