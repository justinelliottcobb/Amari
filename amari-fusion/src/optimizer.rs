...
impl<T: Float> TropicalDualOptimizer<T> {
        ...
        F: Fn(&TropicalDualClifford<T, DIM>) -> DualNumber<T>,
    {
        ...
        for _ in 0..10 { // Limited iterations for warm-up
            ...
            let max_idx = support.iter()
                .max_by(|&&a, &&b| {
                    current.tropical.get(a).value()
    ...
    fn apply_gradient_update<const DIM: usize>(
        ...
        for (i, &v) in velocity.iter().enumerate() {
            if i < 8 { // Limit to dual multivector size
                let current = result.dual.get(i);
                let new_val = DualNumber::variable(
                    T::from(current.real).unwrap_or(T::zero()) + v
                );
...
impl core::fmt::Display for OptimizationError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            OptimizationError::NumericalInstability => 
                write!(f, "Numerical instability detected"),
            OptimizationError::InvalidInitialPoint => 
                write!(f, "Invalid initial point"),
