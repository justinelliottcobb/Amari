//! σ-algebras (sigma-algebras) for measure theory
//!
//! A σ-algebra on a set X is a collection Σ of subsets of X satisfying:
//! 1. X ∈ Σ (whole space is measurable)
//! 2. A ∈ Σ ⟹ Aᶜ ∈ Σ (closed under complements)
//! 3. {Aₙ}ₙ₌₁^∞ ⊂ Σ ⟹ ⋃ₙ₌₁^∞ Aₙ ∈ Σ (closed under countable unions)
//!
//! These properties imply:
//! - ∅ ∈ Σ (empty set is measurable)
...
/// 2. If `A ∈ Σ` then `complement(A) ∈ Σ`
/// 3. If `{Aₙ} ⊂ Σ` then `countable_union({Aₙ}) ∈ Σ`
///
/// # Safety
///
/// The implementation must ensure mathematical correctness of the σ-algebra
...
pub trait SigmaAlgebra {
    ...
    /// Check if a set is measurable (belongs to the σ-algebra)
    ///
    /// # Arguments
    ///
    ...
    /// Check if the whole space is in the σ-algebra
    ///
    /// This must always return `Ok(true)` for a valid σ-algebra.
    fn contains_whole_space(&self) -> Result<bool> {
        Ok(true)
    }

    /// Check if the empty set is in the σ-algebra
    ///
    /// This must always return `Ok(true)` for a valid σ-algebra,
    /// since ∅ = Xᶜ and X ∈ Σ.
    ...
    ///
    /// For a measurable set A, returns Aᶜ = X \ A.
    ///
    /// # Arguments
    ///
    ...
    ///
    /// For measurable sets A and B, returns A ∪ B.
    ///
    /// # Arguments
    ///
    ...
    /// For measurable sets A and B, returns A ∩ B.
    ///
    /// By De Morgan's law: A ∩ B = (Aᶜ ∪ Bᶜ)ᶜ
    ///
    /// # Arguments
    ///
    /// * `a` - First measurable set
    ...
    ///
    /// For measurable sets A and B, returns A \ B = A ∩ Bᶜ.
    ///
    /// # Arguments
    ///
    /// * `a` - First measurable set
...
/// Borel σ-algebra on ℝⁿ
///
/// The Borel σ-algebra is the smallest σ-algebra containing all open sets in ℝⁿ.
...
/// - All countable unions of closed sets (Fσ sets)
/// - All countable intersections of open sets (Gδ sets)
/// - Much more complex sets...
///
/// # Properties
///
/// - The Borel σ-algebra is **incomplete**: there exist non-measurable
///   subsets of Borel null sets
/// - Completion yields the Lebesgue σ-algebra
/// - Every continuous function f: ℝⁿ → ℝᵐ is Borel measurable
///
/// # Examples
///
/// ```
...
impl BorelSigma {
    /// Create a new Borel σ-algebra on ℝⁿ
    ///
    /// # Arguments
    ///
...
/// # Completion Process
///
/// Given the Borel σ-algebra ℬ and Lebesgue measure λ, the Lebesgue
/// σ-algebra consists of sets of the form:
///
/// A = B ∪ N
///
/// where:
/// - B ∈ ℬ (Borel set)
/// - N ⊆ N₀ for some Borel set N₀ with λ(N₀) = 0
///
/// # Properties
///
/// - The Lebesgue σ-algebra is **complete**: all subsets of null sets
...
/// - The Lebesgue σ-algebra is strictly larger than the Borel σ-algebra
///
/// # Examples
///
/// ```
...
impl LebesgueSigma {
    /// Create a new Lebesgue σ-algebra on ℝⁿ
    ///
    /// # Arguments
    ///
    /// * `dimension` - The dimension n of the space ℝⁿ
...
/// Power set σ-algebra
///
/// The power set 𝒫(X) of a set X is the collection of all subsets of X.
/// This is the largest possible σ-algebra on X.
///
/// # Properties
///
/// - Every subset is measurable
/// - Trivially satisfies all σ-algebra axioms
/// - Useful for finite or countable spaces
/// - Not practical for uncountable spaces (e.g., ℝ with counting measure)
///
/// # Examples
///
/// ```
...
impl<X> PowerSet<X> {
    /// Create a new power set σ-algebra
    pub fn new() -> Self {
        Self {
...
/// Trivial σ-algebra
///
/// The trivial σ-algebra on a set X consists of only two sets: {∅, X}.
/// This is the smallest possible σ-algebra on X.
///
/// # Properties
///
/// - Only the empty set and whole space are measurable
/// - Every function is measurable with respect to the trivial σ-algebra
...
impl<X> TrivialSigma<X> {
    /// Create a new trivial σ-algebra
    pub fn new() -> Self {
        Self {
            _phantom: PhantomData,
        }
